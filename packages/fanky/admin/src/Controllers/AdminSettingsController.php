...
//            $file->move(base_path() . $setting::UPLOAD_PATH, $file_name);
            ...
            $file->move(base_path() . $setting::UPLOAD_PATH, $file_name);
            if ($setting->code == 'main_about') {
              $file1080 = $uniq . '--1080' . '.' . $file->getClientOriginalExtension();
              copy(base_path() . $setting::UPLOAD_PATH . $file_name, base_path() . $setting::UPLOAD_PATH . $file1080);
              Image::make(base_path($setting::UPLOAD_PATH . $file1080))
                ->resize(1080, 608, function ($constraint) {
            ...
            if ($file = Request::file($item)) {
              ...
              if ($setting->code == 'main_slider') {
                ...
                Image::make(base_path($setting::UPLOAD_PATH . $file1080))
                  ...
                  ->save(null, 50);
              } elseif($setting->code == 'our_certificates') {
                $filePrev = $uniq . '-prev' . '.' . $file->getClientOriginalExtension();
                copy(base_path() . $setting::UPLOAD_PATH . $file_name, base_path() . $setting::UPLOAD_PATH . $filePrev);
                Image::make(base_path($setting::UPLOAD_PATH . $filePrev))
                  ->resize(1080, 608, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                  })
